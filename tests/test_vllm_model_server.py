...
class TestVLLMModelServerIntegration:
    ...
    def mlrun_project_lifecycle(self):
        ...
        try:
            ...
            project = mlrun.get_or_create_project(
                ...
                context="../")
            print(
                f"MLRun project '{project_name}' created/retrieved successfully")
            ...
            if project:
                try:
                    ...
                    print(
                        f"MLRun project '{project_name}' deleted successfully")
                except Exception as e:
                    print(
                        f"Warning: Could not delete project '{project_name}': {e}")
    ...
    @pytest.fixture    
    def mistral_init_params(self, mlrun_context):
        """Initialization parameters for Mistral model."""
        return {
            'context': mlrun_context,
            'name': 'Mistral-7B-Instruct-v0.2-AWQ',
            'model_path': '/data/.cache/huggingface/Mistral-LLM',
    ...
    def test_store_model(self, tiny_init_params):
        ...
        project = mlrun.get_or_create_project(
            name=tiny_init_params['context'].project)
    ...
    def test_download_tokenizer(self, tiny_init_params):
        """Test that _download_tokenizer downloads tokenizer files."""
        server = VLLMModelServer(**tiny_init_params)
        temp_dir = server._download_tokenizer()

        # Verify that the temporary directory was created
        assert os.path.exists(temp_dir)

        # Check if expected tokenizer files exist in the temp directory
        expected_files = [
            "tokenizer.json",
    ...
    def test_offline_inference(self, tiny_init_params):
        """Test that run_offline_inference runs inference correctly."""
        server = VLLMModelServer(**tiny_init_params)
        ...
        outputs = server.offline_inference(prompts, sampling_params)
        # Verify that outputs are returned
