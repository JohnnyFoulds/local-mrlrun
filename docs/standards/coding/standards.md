...
The purpose of this document is to establish a set of coding standards and best practices for developers working on the AI Booster Plus project.

In terms of coding style specifically, the conventions outlined in the [PEP 8 -- Style Guide for Python Code](https://peps.python.org/pep-0008/) will be followed. This document supplements PEP 8 with additional guidelines specific to the AI Booster Plus project.

## Source Control

All code must be committed to the project's [Git repository](https://gitlab.com/vodacomsa/ai-booster-plus) hosted on GitLab.

Please apply for the `VcIT_GitLab_AI_Booster_Plus` Permission  with the `VcIT_GitLab_AI_Booster_Plus_Write` Profile in [WhitePages](https://whitepages.ent.vodacom.co.za/)
...
└── ai-booster-plus/                    # Main Project Group
    ...
    │   ├── aib-ml-tools/
    │   └── aib-ml-utilities/
    ├── MLRun/                          # Subgroup for the MLRun Orchestration
    │   ├── aib-mlrun-genai-functions/
    │   │── aib-mlrun-genai-images/     # Custom Docker images for MLRun functions
    │   ├── aib-mlrun-ml-functions/
    │   └── aib-mlrun-ml-images/        # Custom Docker
    │── ZenML/
    │   ├── aib-zenml-genai-functions/
    │   │── aib-zenml-genai-images/
...
1. Commit messages must be clear and descriptive, following the [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) format  
   ...
   The `type` should follow the Conventional Commits vocabulary (e.g. `feat`, `fix`, `refactor`, `docs`, `chore`) [4].
